  ...
  <div>
    ...
    <div class="cnt-wrap order-info-wrap">
      <div class="line-title">
        <span>订单信息</span>
        <el-button type="primary" v-if="entryDetail.status !== 4 && entryDetail.status !== 6" size="small" @click="editOrderInfo">编辑</el-button>
      </div>
    ...
    <div class="cnt-wrap candidates-wrap">
      <div class="line-title">
        <span>候选人信息</span>
        <el-button v-if="entryDetail.status >= 3 && entryDetail.status !== 4 && entryDetail.status !== 6" type="primary" size="small" @click="recommendCandidate">推荐候选人</el-button>
      </div>
      <div class="table-list-wrap">
        ...
        <el-form :inline="true" ref="searchForm" :model="searchForm" class="form-inline" label-width="100px">
          ...
          <el-form-item label="候选人状态" prop="status">
            <el-select v-model="searchForm.status" clearable>
              ...
              <el-option label="已入职" value="6"></el-option>
              <el-option label="offer未接受" value="7"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
        ...
        <el-table
          ...
          <el-table-column
            ...
            min-width="200"
            align="center">
            <template slot-scope="scope" v-if="entryDetail.adminId !== null">
              ...
              <el-button v-if="scope.row.recommendStatus === 4 && entryDetail.status !== 4 && entryDetail.status !== 6" type="primary" size="small" @click="recommendAction(scope.row)">入职状态</el-button>
              <el-button v-if="scope.row.recommendStatus === 5" type="primary" size="small" @click="recommendAction(scope.row)">面试信息</el-button>
              <el-button v-if="scope.row.recommendStatus === 6 && entryDetail.status !== 4 && entryDetail.status !== 6" type="primary" size="small" @click="recommendAction(scope.row)">添加入职信息</el-button>
            </template>
          </el-table-column>
          <el-table-column
            prop="interviewTime"
    ...
    <el-dialog title="编辑订单" :visible.sync="editOrderInfoDialog" width="35%" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-form :model="orderInfoForm" ref="orderInfoForm">
        ...
        <el-form-item label="选择职级" labelWidth="120px">
          <el-select v-model="orderInfoForm.jobRank" clearable disabled>
    ...
    <el-dialog title="推荐候选人" :visible.sync="candidateInfoDialog" width="35%" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-form :model="candidateInfoForm" ref="candidateInfoForm" :rules="rules">
        <el-form-item label="附件简历：" labelWidth="100px" prop="uploadedResume">
          ...
          <a :href="candidateInfoForm.uploadedResume | filterFileAddress" class="text-link" target="_blank">{{candidateInfoForm.fileName}}</a>
    ...
    <!-- 设置入职状态 -->
    <el-dialog title="设置入职状态" :visible.sync="setEntryStatusDialog" width="35%">
      <el-form :rules="rules">
        <el-form-item label="请选择状态：" label-width="140px">
          <el-radio-group v-model="entryStatus">
            <el-radio :label="6">已入职</el-radio>
            <el-radio :label="8">面试未通过</el-radio>
            <el-radio :label="7">不接受Offer</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="offer年薪(选填)：" label-width="140px" v-if="entryStatus === 6" prop="offerSalary">
          <!-- offer年薪 -->
          <el-input type="text" v-model="entryDetail.offerSalary"></el-input>
        </el-form-item>
        <el-form-item label="上传offer：" labelWidth="140px" v-if="entryStatus === 6">
          <el-upload
            class="file-uploader"
            :multiple="false"
            action="http://upload.qiniup.com/"
            ...
            <el-button size="small" type="primary">点击上传</el-button>
            <span v-if="fileUploading">文件上传中...</span>
            <a :href="entryDetail.uploadedResume" class="text-link">{{entryDetail.fileName}}</a>
          </el-upload>
        </el-form-item>
        <el-form-item label="需扣入职金额(元)：" label-width="140px" v-if="entryStatus === 6">
          <!-- 价格只做显示用 -->
          <el-input type="text" v-model="entryDetail.price" disabled></el-input>
        </el-form-item>
        <el-form-item label="公司账户余额(元)：" label-width="140px" v-if="entryStatus === 6">
          <!-- 价格只做显示用 -->
          <el-input type="text" v-model="entryPriceInfo.balance" disabled></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer center">
        <el-button type="primary" @click="sureSetEntryStatus" :disabled="loading">{{loading ? '提交中...' : '确 定'}}</el-button>
      </div>
    </el-dialog>

    <!-- 添加入职信息 -->
    <el-dialog title="添加入职信息" :visible.sync="entryInfoDialog" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-form v-model="entryInfoForm" ref="entryInfoForm">
        <el-form-item label="候选人姓名" label-width="140px">
          <el-input type="text" v-model="entryInfoForm.entryName"></el-input>
        </el-form-item>
        <el-form-item label="联系方式" label-width="140px">
          <el-input type="text" v-model="entryInfoForm.entryContactMobile"></el-input>
        </el-form-item>
        <el-form-item label="职称" label-width="140px">
          <el-input type="text" v-model="entryInfoForm.entryContactJobTitle"></el-input>
        </el-form-item>
        <el-form-item label="入职时间" label-width="140px">
          <el-date-picker
            v-model="entryInfoForm.entryTime"
            type="date"
            placeholder="选择日期"
            value-format="yyyy-MM-dd HH:mm:ss">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="offer" label-width="140px">
          <el-upload
            class="file-uploader"
            :multiple="false"
            action="http://upload.qiniup.com/"
            ...
            :on-error="handleOfferError"
            :on-progress="handleOfferProgress"
            :data="uploadData"
            :before-upload="beforeOfferUpload">
            <el-button size="small" type="primary">点击上传</el-button>
            <span v-if="fileUploading">文件上传中...</span>
          </el-upload>
          <a :href="entryInfoForm.offerAddress | filterFileAddress" class="text-link" v-if="entryInfoForm.offerAddress !== ''"  target="_blank">{{entryInfoForm.fileName}}</a>
        </el-form-item>
        <el-form-item label="开票信息" label-width="140px">
          <el-input type="textarea" v-model="entryInfoForm.kpInfo" :autosize="{ minRows: 3}"></el-input>
        </el-form-item>
        <el-form-item label="付款信息" label-width="140px">
          <el-input type="textarea" v-model="entryInfoForm.payInfo" :autosize="{ minRows: 3}"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer center">
        <el-button type="plan" @click="cancelEntryInfo">取 消</el-button>
        <el-button type="primary" @click="sureEntryInfo">确 定</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog title="提醒" :visible.sync="noMoneyTipDialog" width="35%">
      ...
      <div slot="footer" class="dialog-footer center">
        <el-button type="primary" @click="sendNoMoneyNotice">{{emailSending ? '正在发送...' : '发送通知'}}</el-button>
...
export default {
  data () {
    return {
      ...
      rules: {
        ...
        offerSalary: [
          { pattern: Global.REGEX.NUMGTZERO, message: '年薪必须为数字', trigger: 'blur' }
        ]
      },
      uploadData: {
        token: '',
  ...
  methods: {
    fetchEntryDetail () {
      let _vue = this
      _vue.$http.get('/api/v1.0/ensure/entry?id=' + _vue.entryId).then(function (response) {
    ...
    fetchCandidate () {
      // 查询推荐候选人
      let _vue = this
      _vue.$http.get('/api/v1.0/ensure/entry/recommends?ensure_id=' + _vue.entryId +
    ...
    fetchRecommendInfo (recommendId) {
      // 获取候选人推荐信息，包括面试信息
      let _vue = this
      _vue.$http.get('/api/v1.0/ensure/entry/recommend?recommend_id=' + recommendId).then((response) => {
    ...
    fetchPriceTotal () {
      ...
      _vue.$http.get('/api/v1.0/ensure/account/balance?recommend_id=' + _vue.currentCandidateInfo.id).then((response) => {
        ...
        _vue.noMoneyRecommendId = _vue.currentCandidateInfo.id
      })
    },
    fetchPriceDetail () {
      // 获取价格详情
      let _vue = this
      _vue.$http.get('/api/v1.0/ensure/entry/cost?ensure_id=' + _vue.entryId).then((response) => {
    ...
    sureCandidateInfo (formName) {
      ...
      _vue.$refs[formName].validate((valid) => {
        if (valid) {
          _vue.$http.post('/api/v1.0/ensure/entry/recommend', postData).then(() => {
            _vue.$message({
              type: 'success',
              message: '推荐成功',
              center: true
    ...
    recommendAction (row, status) {
      ...
      if (row.recommendStatus === 2) {
      ...
      } else if (row.recommendStatus === 4) {
        // 入职状态
        let _vue = this
        _vue.$http.get('/api/v1.0/ensure/account/balance?recommend_id=' + _vue.currentCandidateInfo.id).then((response) => {
          _vue.entryPriceInfo.balance = response.data.data.amount
          // 查询余额后，余额和待扣款需要做一下对比，钱不够就提示
          if (_vue.entryPriceInfo.balance < _vue.entryDetail.price) {
            _vue.noMoneyTipDialog = true
            _vue.noMoneyRecommendId = _vue.currentCandidateInfo.id
          } else {
            _vue.setEntryStatusDialog = true
          }
        }).catch(() => {
          // 出错表示账户余额不足，给出提示：余额不足，请提醒HR及时充值
          _vue.noMoneyTipDialog = true
        ...
        this.interviewInfoDialog = true
      } else if (row.recommendStatus === 6) {
        // 添加入职信息
        this.entryInfoForm.recommendId = row.id
        this.entryInfoForm.entryName = row.entryName || row.name
        this.entryInfoForm.entryContactMobile = row.entryContactMobile || row.mobile
        this.entryInfoForm.entryContactJobTitle = row.entryContactJobTitle || ''
        this.entryInfoForm.entryTime = (row.entryTime || '').split(' ')[0]
        this.entryInfoForm.offerAddress = row.offerAddress || ''
        if (this.entryInfoForm.offerAddress !== '') {
          this.entryInfoForm.fileName = '查看offer'
        }
        this.entryInfoForm.kpInfo = row.kpInfo || ''
        this.entryInfoForm.payInfo = row.payInfo || ''
    ...
    sureSetEntryStatus () {
      ...
      if (_vue.loading) {
        ...
        _vue.loading = true
        _vue.$http.post('/api/v1.0/ensure/entry/recommend/offer', {
          recommendId: _vue.currentCandidateInfo.id,
          recommendStatus: _vue.entryStatus,
          offerSalary: _vue.entryDetail.offerSalary,
          offerAddress: _vue.entryDetail.uploadedResume
        }).then(() => {
          _vue.$message({
            type: 'success',
            message: '入职状态设置成功',
          ...
          _vue.setEntryStatusDialog = false
          _vue.fetchCandidate()
          _vue.loading = false
        }).catch(() => {
          _vue.$message({
    ...
    sureEntryInfo () {
      ...
      _vue.$http.post('/api/v1.0/ensure/entry/recommend/offer', {
        recommendId: _vue.currentCandidateInfo.id,
        entryName: _vue.entryInfoForm.entryName,
        entryContactMobile: _vue.entryInfoForm.entryContactMobile,
        entryContactJobTitle: _vue.entryInfoForm.entryContactJobTitle,
        entryTime: _vue.entryInfoForm.entryTime,
        offerAddress: _vue.entryInfoForm.offerAddress,
        kpInfo: _vue.entryInfoForm.kpInfo,
        payInfo: _vue.entryInfoForm.payInfo
      }).then(() => {
        _vue.$message({
          type: 'success',
          message: '入职信息提交成功',
    ...
    sureEditOrderInfo () {
      // TODO: 提交编辑订单信息, 表单验证
      let _vue = this
      _vue.$http.put('/api/v1.0/ensure/entry', _vue.orderInfoForm).then(() => {
        _vue.$message({
          type: 'success',
          message: '订单信息编辑成功',
          center: true
    ...
    handleFileSuccess (res, file) {
      ...
      this.entryDetail.fileName = file.name
      this.entryDetail.uploadedResume = res.key
      this.fileUploading = false
    },
    handleFileProgress () {
      // 文件上传中
    ...
    handleOfferSuccess (res, file) {
      // offer上传成功
      this.entryInfoForm.fileName = file.name
      this.entryInfoForm.offerAddress = res.key
      this.fileUploading = false
    },
    handleOfferError () {
      // offer上传失败
    ...
    viewActionRecord (row) {
      ...
      _vue.$http.get('/api/v1.0/ensure/entry/recommend/opts?recommend_id=' + row.id).then((response) => {
    ...
    sendNoMoneyNotice () {
      ...
      if (_vue.emailSending) {
        ...
        if (_vue.noMoneyRecommendId !== '') {
          _vue.emailSending = true
          _vue.$http.post('/api/v1.0/ensure/account/balance/less', {
            recommendId: _vue.noMoneyRecommendId
          }).then(() => {
            _vue.emailSending = false
            _vue.noMoneyTipDialog = false
            _vue.$message({
              type: 'success',
              message: '已发送邮件通知',
              center: true
