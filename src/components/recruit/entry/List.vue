  ...
  <div class="table-list-wrap">
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <router-link class="text-link" :to="{ name: 'entryDetail', params: { id: scope.row.ensureId }}">{{scope.row.orderNo}}</router-link>
        </template>
      </el-table-column>
      <el-table-column
        label="订单状态"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.adminId !== null && scope.row.entryCount === 0 && scope.row.status !== 6" type="primary" size="small" @click="handleCloseOrder(scope.row)">关闭订单</el-button>
          <el-button v-if="scope.row.adminId !== null && scope.row.entryCount >= 1 && scope.row.status !== 4" type="primary" size="small" @click="handleFinishOrder(scope.row)">完成订单</el-button>
    ...
    <el-dialog title="完成订单" :visible.sync="finishOrderDialog" width="35%">
      <el-form>
        ...
        <el-form-item label="入职人数：" label-width="140px">
          <span>{{currentOrder.entryCount}}</span>
        </el-form-item>
        <el-form-item label="服务费用：" label-width="140px">
          <span>{{orderTotalMoney}}</span>
        </el-form-item>
        <div class="dialog-bottom-tip left">自动发送确认邮件至HR</div>
...
export default {
  ...
  methods: {
    fetchTableData () {
      // 获取订单列表
      let _vue = this
      _vue.$http.get('/api/v1.0/ensure/entries?page=' + _vue.pager.curPage +
    ...
    handleFinishOrder (row) {
      // 点击完成订单
      let _vue = this
      _vue.finishOrderDialog = true
      _vue.currentOrder = row
      _vue.$http.get('/api/v1.0/ensure/entry/cost_total_money?ensure_id=' + row.ensureId).then((response) => {
        _vue.orderTotalMoney = response.data.data.money
      }).catch(() => {
        _vue.$message({
          type: 'error',
          message: '请稍后重试',
    ...
    handleOrderRecord (row) {
      ...
      _vue.$http.get('/api/v1.0/ensure/entry/opts?ensure_id=' + row.ensureId).then((response) => {
    ...
    sureAssignAdviser () {
      ...
      if (_vue.assignAdviserForm.adminId === '') {
        ...
        _vue.$http.post('/api/v1.0/ensure/entry/manager', {
    ...
    sureCloseOrder () {
      ...
      if (_vue.loading) {
        ...
        _vue.loading = true
        _vue.$http.post('/api/v1.0/ensure/entry/close', {
    ...
    sureFinishOrder () {
      ...
      if (_vue.loading) {
        ...
        _vue.loading = true
        _vue.$http.post('/api/v1.0/ensure/entry/complete', {
          ensureId: _vue.currentOrder.ensureId
        }).then(() => {
          _vue.$message({
            type: 'success',
