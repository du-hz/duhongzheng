<template>
    <div class="table-list-wrap mt-table-list-wrap">
        ...
        <el-table :data="tableData" ref="multipleTable" size="mini" @selection-change="handleSelectionChange" header-row-class-name="tableheader" @sort-change="sortList" border style="width: 100%;" max-height="500">
            ...
            <el-table-column label="行为展示" :show-overflow-tooltip="true" width="130" prop="english_name" fixed>
                <template slot-scope="scope">
...
export default {
  ...
  methods: {
    createAfter () {
      ...
      this.searchForm.companyName = this.$route.query.company
      this.fetchTableData()
    },
    getToken () {
      ...
      this.http.get('/api/v1.0/crm/mock/login', {
        params: {
    ...
    fetchChain () {
      // 查询产业链
      let _vue = this
      this.http.get('/api/v1.0/customer/industrychains').then((response) => {
    ...
    fetchTableData () {
      // 获取人才列表
      let _vue = this
      this.http.get('/api/v1.0/candidate/search?page=' + _vue.pager.curPage + // 第几页
    ...
    keyDialogShow () {
      ...
      if (Global.isBlankStr(searchStr)) {
        ...
        if (!_vue.saveOrUpdate) {
          ...
          this.http.put('/api/v1.0/candidate/query-params', {
            id: _vue.queryId,
            name: _vue.keyName,
    ...
    keyDialogSure () {
      ...
      if (Global.isBlankStr(_vue.keyName)) {
        ...
        this.http.post('/api/v1.0/candidate/query-params', {
          name: _vue.keyName,
          query: queryStr,
          searchString: searchStr,
          ct: _vue.totalNum
    ...
    foldDialogSure () {
      ...
      if (_vue.fold.id === undefined) {
        this.http.post('/api/v1.0/candidate/floder', {
    ...
    contactShow (row) {
      ...
      this.http.get('/api/v1.0/candidate/contacts?id=' + row.talentPoolId).then((response) => {
        let resData = response.data.data
    ...
    addPepToFold: function (foldId) {
      ...
      this.http.post('/api/v1.0/candidate/floder/candidate', {
    ...
    deletePep () {
      ...
      this.http.delete('/api/v1.0/candidate/floder/candidate?candidateIds=' + candidateIds).then((response) => {
        this.$message({
          type: 'success',
          message: '操作成功!'
        })
    ...
    fetchFolds () {
      // 查询所有收藏文件夹
      let _vue = this
      this.http.get('/api/v1.0/candidate/floder?page=1&rows=9999').then((response) => {
    ...
    viewAction (row) {
      ...
      this.$router.push({
        ...
        query: {
          ...
          account: _vue.searchs.account,
          username: _vue.searchs.username,
          companyName: _vue.searchs.companyName,
          source: _vue.searchs.source,
          signature: _vue.searchs.signature,
          searchTalentPoolId: _vue.searchs.searchTalentPoolId
        }
      })
    }
  }
